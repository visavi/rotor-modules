...
@section('settings')
    <form method="post" action="{{ route('antimat.settings.update') }}">
        ...
        <div class="mb-3{{ hasError('sets[antimat_replace]') }}">
            ...
            <span class="text-muted fst-italic">{{ __('antimat::antimat.settings_replace_hint') }}</span>
        </div>

        <div class="form-check form-switch mb-3">
            <input type="hidden" value="0" name="sets[antimat_whole_word]">
            <input type="checkbox" role="switch" class="form-check-input" value="1" name="sets[antimat_whole_word]" id="antimat_whole_word"{{ ! empty($settings['antimat_whole_word']) ? ' checked' : '' }}>
            <label class="form-check-label" for="antimat_whole_word">{{ __('antimat::antimat.settings_whole_word') }}</label>
            <div class="text-muted fst-italic">{{ __('antimat::antimat.settings_whole_word_hint') }}</div>
        </div>
