...
return new class extends Migration {
    ...
    {
        if (Schema::getColumnType('paid_adverts', 'created_at') === 'datetime') {
            return;
        }

        $toDt = static fn ($v) => $v ? Date::createFromTimestamp($v, config('app.timezone'))->format('Y-m-d H:i:s') : null;

        Schema::table('paid_adverts', function (Blueprint $table) {
            $table->dateTime('created_at_dt')->nullable();
            $table->dateTime('deleted_at_dt')->nullable();
        });
        $this->convert('paid_adverts', ['created_at', 'deleted_at'], static fn ($r) => [
            'created_at_dt' => $toDt($r->created_at),
            'deleted_at_dt' => $toDt($r->deleted_at),
        ]);
        Schema::table('paid_adverts', fn (Blueprint $table) => $table->dropColumn(['created_at', 'deleted_at']));
        Schema::table('paid_adverts', function (Blueprint $table) {
