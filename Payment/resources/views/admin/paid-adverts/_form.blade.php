    ...
    <div class="mb-3{{ hasError('place') }}">
        ...
        <select class="form-select" name="place" id="place">
            @foreach ($places as $place)
                ...
                <option value="{{ $place }}"{{ $selected }}>{{ __('payment::payments.paid_adverts.' . $place) }}</option>
            @endforeach
        </select>
        ...
        <div class="js-advert-list">
            ...
            @for ($i = 0; $i < max(1, count($names)); $i++)
                @if ($i === 0)
                    ...
                    <input type="text" name="names[]" class="form-control" id="names{{ $i }}" value="{{ $names[$i] ?? '' }}" maxlength="35" placeholder="{{ __('payment::payments.paid_adverts.name') }}">
                @else
                    <div class="input-group mt-1 js-advert-append">
                        <input class="form-control" name="names[]" type="text" value="{{ $names[$i] ?? '' }}" maxlength="35" placeholder="{{ __('payment::payments.paid_adverts.name') }}">
    ...
    <script type="module">
        document.querySelector('.js-advert-add')?.addEventListener('click', function (e) {
            ...
            document.querySelector('.js-advert-list').insertAdjacentHTML('beforeend',
                '<div class="input-group mt-1 js-advert-append">' +
                '<input class="form-control" name="names[]" type="text" value="" maxlength="35" placeholder="<?= __('payment::payments.paid_adverts.name') ?>">' +
                '<span class="input-group-text">' +
                    '<a class="js-advert-remove" href="#"><i class="fa fa-times"></i></a>' +
                '</span>' +
                '</div>');
