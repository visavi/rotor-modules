<form action="/payments/calculate" method="post">
            ...
            @for ($i = 0; $i < max(1, count($names)); $i++)
                ...
                @else
                    <div class="input-group mt-1 js-advert-append">
                        <input class="form-control{{ hasError('names.'. $i) }}" name="names[]" type="text" value="{{ $names[$i] ?? '' }}" maxlength="35" placeholder="{{ __('payment::payments.paid_adverts.name') }}">
    ...
    <div class="mb-3">
        <label for="comment" class="form-label">{{ __('main.comment') }}:</label>
        <textarea class="form-control{{ hasError('comment') }}" id="comment" rows="5" name="comment">{{ old('comment', $advert->comment) }}</textarea>
