...
return new class extends Migration {
    ...
    {
        if (Schema::getColumnType('gifts', 'created_at') === 'datetime') {
            return;
        }

        $toDt = static fn ($v) => $v ? Date::createFromTimestamp($v, config('app.timezone'))->format('Y-m-d H:i:s') : null;

        // gifts: только created_at
        Schema::table('gifts', fn (Blueprint $table) => $table->dateTime('created_at_dt')->nullable());
        $this->convert('gifts', ['created_at'], static fn ($r) => ['created_at_dt' => $toDt($r->created_at)]);
        Schema::table('gifts', fn (Blueprint $table) => $table->dropColumn('created_at'));
        Schema::table('gifts', fn (Blueprint $table) => $table->renameColumn('created_at_dt', 'created_at'));

        // gifts_users: created_at + deleted_at (срок действия)
        Schema::table('gifts_users', function (Blueprint $table) {
            $table->dateTime('created_at_dt')->nullable();
            $table->dateTime('deleted_at_dt')->nullable();
        });
        $this->convert('gifts_users', ['created_at', 'deleted_at'], static fn ($r) => [
            'created_at_dt' => $toDt($r->created_at),
            'deleted_at_dt' => $toDt($r->deleted_at),
        ]);
        Schema::table('gifts_users', fn (Blueprint $table) => $table->dropColumn(['created_at', 'deleted_at']));
        Schema::table('gifts_users', function (Blueprint $table) {
    ...
    public function down(): void
    {
        ...
        Schema::table('gifts', fn (Blueprint $table) => $table->integer('created_at_int')->nullable());
        $this->convert('gifts', ['created_at'], static fn ($r) => ['created_at_int' => $toInt($r->created_at)]);
        Schema::table('gifts', fn (Blueprint $table) => $table->dropColumn('created_at'));
        Schema::table('gifts', fn (Blueprint $table) => $table->renameColumn('created_at_int', 'created_at'));

        Schema::table('gifts_users', function (Blueprint $table) {
            $table->integer('created_at_int')->nullable();
            $table->integer('deleted_at_int')->nullable();
        });
        $this->convert('gifts_users', ['created_at', 'deleted_at'], static fn ($r) => [
