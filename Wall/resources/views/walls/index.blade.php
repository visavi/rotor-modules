...
@section('title', __('wall::walls.wall_posts_login', ['login' => $user->getName()]))
...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('wall::walls.wall_posts') }}</li>
        </ol>
    </nav>
@endsection
                    ...
                    @if (getUser())
                        <div class="js-actions">
                            @if (getUser('id') !== $data->author_id)
                                ...
                                <a href="#" data-ajax data-ajax-url="/ajax/complaint" data-ajax-confirm="{{ __('main.confirm_complaint') }}" data-ajax-icon="fa fa-check text-muted" data-type="{{ $data->getMorphClass() }}" data-id="{{ $data->id }}" data-page="{{ $messages->currentPage() }}" rel="nofollow" title="{{ __('main.complain') }}"><i class="fa fa-bell text-muted"></i></a>
                            @endif

                            @if (isAdmin() || getUser('id') === $user->id)
                                <a href="#" data-ajax data-ajax-url="{{ route('walls.delete', ['login' => $data->user->login]) }}" data-ajax-confirm data-ajax-remove=".section" data-id="{{ $data->id }}" data-bs-toggle="tooltip" title="{{ __('main.delete') }}"><i class="fa fa-times text-muted"></i></a>
                            @endif
                        </div>
                    @endif
                </div>
