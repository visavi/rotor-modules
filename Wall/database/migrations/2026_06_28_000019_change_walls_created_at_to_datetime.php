<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Date;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Schema;

return new class extends Migration {
    /**
     * Реальные имена индексов по колонкам: базы со старого движка держат
     * индекс как <table>_<column>, а dropIndex(['column']) ищет
     * <table>_<column>_index и падает с 1091 на чужом имени.
     */
    private function indexNames(string $table, array $columns): array
    {
        $names = [];

        foreach (Schema::getIndexes($table) as $index) {
            if (! $index['primary'] && ! $index['unique'] && in_array($index['columns'], $columns, true)) {
                $names[] = $index['name'];
            }
        }

        return $names;
    }

    /**
     * Чанковая конверсия с транзакцией на чанк: createFromTimestamp/parse сохраняют
     * историческую таймзону (старый DST), а батч-коммит убирает fsync-на-строку.
     */
    private function convert(string $table, array $cols, callable $map): void
    {
        DB::table($table)->select(array_merge(['id'], $cols))->orderBy('id')
            ->chunkById(5000, function ($rows) use ($table, $map) {
                DB::transaction(function () use ($rows, $table, $map) {
                    foreach ($rows as $row) {
                        DB::table($table)->where('id', $row->id)->update($map($row));
                    }
                });
            });
    }

    /**
     * Создаёт только отсутствующие временные колонки: упавшая миграция могла
     * оставить их с прошлого запуска, повторный запуск не должен падать.
     */
    private function addTempColumns(string $table, string $type, array $cols): void
    {
        $missing = array_filter($cols, static fn ($col) => ! Schema::hasColumn($table, $col));

        if ($missing) {
            Schema::table($table, static function (Blueprint $blueprint) use ($type, $missing) {
                foreach ($missing as $col) {
                    $blueprint->{$type}($col)->nullable();
                }
            });
        }
    }

    public function up(): void
    {
        if (Schema::getColumnType('walls', 'created_at') === 'datetime') {
            return;
        }

        $toDt = static fn ($v) => $v ? Date::createFromTimestamp($v, config('app.timezone'))->format('Y-m-d H:i:s') : null;

        $this->addTempColumns('walls', 'dateTime', ['created_at_dt']);
        $this->convert('walls', ['created_at'], static fn ($r) => ['created_at_dt' => $toDt($r->created_at)]);
        $indexes = $this->indexNames('walls', [['created_at']]);
        Schema::table('walls', function (Blueprint $table) use ($indexes) {
            foreach ($indexes as $index) {
                $table->dropIndex($index);
            }
            $table->dropColumn('created_at');
        });
        Schema::table('walls', fn (Blueprint $table) => $table->renameColumn('created_at_dt', 'created_at'));
        Schema::table('walls', fn (Blueprint $table) => $table->index('created_at'));
    }

    public function down(): void
    {
        if (Schema::getColumnType('walls', 'created_at') !== 'datetime') {
            return;
        }

        $toInt = static fn ($v) => $v ? Date::parse($v, config('app.timezone'))->getTimestamp() : null;

        $this->addTempColumns('walls', 'integer', ['created_at_int']);
        $this->convert('walls', ['created_at'], static fn ($r) => ['created_at_int' => $toInt($r->created_at)]);
        $indexes = $this->indexNames('walls', [['created_at']]);
        Schema::table('walls', function (Blueprint $table) use ($indexes) {
            foreach ($indexes as $index) {
                $table->dropIndex($index);
            }
            $table->dropColumn('created_at');
        });
        Schema::table('walls', fn (Blueprint $table) => $table->renameColumn('created_at_int', 'created_at'));
        Schema::table('walls', fn (Blueprint $table) => $table->index('created_at'));
    }
};
