...
@section('title', __('backup::backup.create_backup'))
...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="/admin/backups">{{ __('backup::backup.backup') }}</a></li>
            <li class="breadcrumb-item active">{{ __('backup::backup.create_backup') }}</li>
        </ol>
    </nav>
@stop
...
@section('content')
    @if ($tables)
        {{ __('backup::backup.total_tables') }}: <b>{{ count($tables) }}</b><br><br>
        ...
        <div class="section-form mb-3 shadow">
            <form action="/admin/backups/create" method="post">
                ...
                @foreach ($tables as $data)
                    ...
                    <div class="form-check">
                        <label class="form-check-label">
                            ...
                            <i class="fa fa-database"></i> <b>{{ $data->Name }}</b> ({{ __('backup::backup.records') }}: {{ $data->Rows }} / {{ __('backup::backup.size') }}: {{ formatSize((int) $data->Data_length) }})
                        </label>
                    </div>
                @endforeach
                ...
                <div class="mb-3{{ hasError('method') }}">
                    <label for="method" class="form-label">{{ __('backup::backup.compress_method') }}:</label>
                    <select class="form-select" id="method" name="method">

                        <option value="none">{{ __('backup::backup.not_compress') }}</option>
    ...
    @else
        {{ showError(__('backup::backup.empty_tables')) }}
    @endif
@stop
