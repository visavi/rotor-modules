...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('offers.index', ['type' => $offer->type]) }}">{{ __('offer::offers.section') }}</a></li>
...
@section('content')
    ...
    @if ($offer->reply)
        <div class="section mb-3 shadow">
            <h5>{{ __('offer::offers.official_response') }}</h5>
            <div class="section-message">
                {{ $offer->getReply() }}<br>
    ...
    <div class="section mb-3 shadow p-0 overflow-hidden" id="comments">
        ...
        <div class="px-3">
            ...
            @foreach ($comments as $comment)
                @include('app/_comment_item', ['action' => route('offers.add-comment', ['id' => $offer->id]), 'closed' => $offer->closed, 'ownerId' => $offer->user_id])
            @endforeach
        </div>
    </div>
@stop
