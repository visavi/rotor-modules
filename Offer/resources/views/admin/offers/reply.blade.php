...
@section('title', __('offer::offers.reply_record'))
...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('offer::offers.reply_record') }}</li>
        </ol>
    </nav>
@stop
            ...
            <div class="mb-3{{ hasError('status') }}">
                ...
                <select class="form-select" name="status" id="status">
                    @foreach ($statuses as $status)
                        ...
                        <option value="{{ $status }}"{{ $selected }}>{{ __('offer::offers.' . $status) }}</option>
                    @endforeach
                </select>
