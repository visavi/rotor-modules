...
@section('content')
    <div class="mb-3">
        <div class="mb-3">
            ...
            <a class="btn btn-{{ $active }} btn-sm" href="{{ route('admin.offers.index', ['type' => 'offer', 'sort' => $sort, 'order' => $order]) }}">{{ __('offer::offers.offers') }} <span class="badge bg-adaptive">{{ $offerCount }}</span></a>

            <?php $active = ($type === 'issue') ? 'primary' : 'adaptive'; ?>
            <a class="btn btn-{{ $active }} btn-sm" href="{{ route('admin.offers.index', ['type' => 'issue', 'sort' => $sort, 'order' => $order]) }}">{{ __('offer::offers.problems') }} <span class="badge bg-adaptive">{{ $issueCount }}</span></a>
        </div>
    </div>
