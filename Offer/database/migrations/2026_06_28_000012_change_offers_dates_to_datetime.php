...
return new class extends Migration {
    ...
    {
        if (Schema::getColumnType('offers', 'created_at') === 'datetime') {
            return;
        }
        ...
        $this->convert('offers', ['created_at', 'updated_at'], static fn ($r) => [
            'created_at_dt' => $toDt($r->created_at),
            'updated_at_dt' => $toDt($r->updated_at),
        ]);
        $indexes = $this->indexNames('offers', [['created_at']]);
        Schema::table('offers', function (Blueprint $table) use ($indexes) {
        ...
        Schema::table('offers', fn (Blueprint $table) => $table->index('created_at'));
    }
