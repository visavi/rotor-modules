...
@section('content')
    @if ($posts->isNotEmpty())
                ...
                <div class="section mb-3 shadow">
                    ...
                    <div class="section-user d-flex align-items-start">
                        <div class="flex-grow-1">
                            ...
                            @if (! $post->active)
                                <span class="badge bg-danger">{{ __('guestbook::guestbook.not_publish') }}</span>
                            @endif
                        </div>
        ...
        @if (isAdmin('boss'))
            <form action="{{ route('admin.guestbook.clear') }}" method="post" onsubmit="return confirm('{{ __('guestbook::guestbook.confirm_delete') }}')">
                @csrf
