    ...
    <div class="section-form mb-3 shadow">
        <form action="{{ route('photos.edit', ['id' => $photo->id, 'page' => $page]) }}" method="post">
            ...
            <div class="mb-3{{ hasError('title') }}">
                ...
                <input type="text" class="form-control" id="inputTitle" name="title" maxlength="{{ setting('photo_title_max') }}" value="{{ old('title', $photo->title) }}" required>
                <div class="invalid-feedback">{{ textError('title') }}</div>
            </div>

            <div class="mb-3{{ hasError('text') }}">
                <label for="text" class="form-label">{{ __('photo::photos.description') }}:</label>
                <textarea class="form-control tiptap" maxlength="{{ setting('photo_text_max') }}" id="text" rows="5" name="text">{{ old('text', $photo->text) }}</textarea>
