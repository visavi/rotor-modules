...
@section('title',  __('transfer::transfers.money_transfer'))

@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="/"><i class="fas fa-home"></i></a></li>
            <li class="breadcrumb-item active">{{ __('transfer::transfers.money_transfer') }}</li>
        </ol>
    </nav>
@stop

@section('content')
    {{ __('transfer::transfers.in_stock') }}: {{ plural(getUser('money'), setting('moneyname')) }}<br><br>
    ...
    @if (getUser('point') >= setting('sendmoneypoint'))
        <div class="section-form mb-3 shadow">
            <form action="/transfers/send" method="post">
                @csrf
                @if ($user)
                    <i class="fas fa-coins"></i> {{ __('transfer::transfers.transfer_for') }} <b>{{ $user->getName() }}</b>:<br><br>
                ...
                <button class="btn btn-primary">{{ __('transfer::transfers.transfer') }}</button>
            </form>
        </div>
    @else
       {{ showError(__('transfer::transfers.transfer_point', ['point' => plural(setting('sendmoneypoint'), setting('scorename'))])) }}
    @endif
@stop
