        ...
        @foreach ($articles as $article)
            <div class="section mb-3 shadow">
                ...
                <div class="section-content mb-2">
                    {{ __('blog::blogs.blog') }}: <a href="{{ route('blogs.blog', ['id' => $article->category_id]) }}">{{ $article->category->name }}</a><br>
                ...
                <i class="fa-regular fa-comment"></i> <a href="{{ route('articles.view', ['slug' => $article->slug]) }}#comments">{{ __('main.comments') }}</a> <span class="badge bg-adaptive">{{ $article->count_comments }}</span>
            </div>
        @endforeach
    @else
