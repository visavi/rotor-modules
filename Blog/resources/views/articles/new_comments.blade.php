        ...
        @foreach ($comments as $comment)
            <div class="section mb-3 shadow">
                <div class="section-title">
                    <i class="fa fa-comment"></i> <a href="{{ route('articles.view', ['slug' => $comment->relate->slug]) }}#comment_{{ $comment->id }}">{{ $comment->title }}</a> <span class="badge bg-adaptive">{{ $comment->count_comments }}</span>
