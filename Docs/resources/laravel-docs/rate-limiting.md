...
Для взаимодействия с ограничителем частоты используется фасад `Illuminate\Support\Facades\RateLimiter`. Его простейший метод `attempt` ограничивает частоту выполнения переданного callback-вызова в течение заданного количества секунд.
