---
git: 0349523baa79130a186ba3fb2285bcc90262d4a5
---
...
> [!NOTE]
> [Laravel Cloud](https://cloud.laravel.com) предлагает полностью управляемую WebSocket-инфраструктуру на базе кластеров Laravel Reverb, позволяя масштабировать и выпускать приложения с Reverb без управления инфраструктурой.
...
Reverb отправляет внутренние события в течение жизненного цикла соединения и обработки сообщений. Вы можете [слушать эти события](/docs/{{version}}/events), чтобы выполнять действия при управлении соединениями или обмене сообщениями.

Reverb отправляет следующие события:

#### `Laravel\Reverb\Events\ChannelCreated`

Отправляется при создании канала. Обычно это происходит, когда первое соединение подписывается на определённый канал. Событие получает экземпляр `Laravel\Reverb\Protocols\Pusher\Channel`.

#### `Laravel\Reverb\Events\ChannelRemoved`

Отправляется при удалении канала. Обычно это происходит, когда последнее соединение отписывается от канала. Событие получает экземпляр `Laravel\Reverb\Protocols\Pusher\Channel`.

#### `Laravel\Reverb\Events\ConnectionPruned`

Отправляется, когда устаревшее соединение удаляется сервером. Событие получает экземпляр `Laravel\Reverb\Contracts\Connection`.

#### `Laravel\Reverb\Events\MessageReceived`

Отправляется, когда сообщение получено от клиентского соединения. Событие получает экземпляр `Laravel\Reverb\Contracts\Connection` и исходную строку `$message`.

#### `Laravel\Reverb\Events\MessageSent`

Отправляется, когда сообщение отправлено клиентскому соединению. Событие получает экземпляр `Laravel\Reverb\Contracts\Connection` и исходную строку `$message`.
