---
git: d9131dd2d7e75604555b1e6c5c106b1e41279e78
---
...
[Посредник](/docs/{{version}}/middleware) `Illuminate\Foundation\Http\Middleware\PreventRequestForgery`, который по умолчанию включен в группу посредников `web`, защищает ваше приложение от подделки межсайтовых запросов с помощью двухуровневого подхода.
...
Если вы создаете SPA, который использует Laravel в качестве серверной части API, вам следует обратиться к [документации Sanctum](/docs/{{version}}/sanctum) для получения информации об аутентификации с помощью вашего API и защите от уязвимостей CSRF.
...
> [!WARNING]
> Заголовок `Sec-Fetch-Site` отправляется браузерами только через защищенные соединения (HTTPS). Если ваше приложение обслуживается не через HTTPS, проверка источника будет недоступна, и посредник вернется к проверке CSRF-токена.
