---
git: 9985cb98046abd10ec840cbca73a8ba940cc2336
---
...
| `monthly`    | Драйвер Monolog на основе `RotatingFileHandler` с ежемесячной ротацией.        |
...
#### Конфигурирование каналов Single, Daily и Monthly

Каналы `single`, `daily` и `monthly` имеют три необязательных параметра конфигурации: `bubble`, `permission` и `locking`.
...
Дополнительно, политику хранения для каналов `daily` и `monthly` можно настроить с помощью параметра конфигурации `max_files`. Переменную окружения `LOG_DAILY_DAYS` также можно использовать для настройки хранения канала `daily`.
