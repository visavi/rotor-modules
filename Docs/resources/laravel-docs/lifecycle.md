...
HTTP-ядро также передает запрос через стек посредников. Они выполняют чтение и запись [HTTP-сессии](/docs/{{version}}/session), определяют, находится ли приложение в режиме обслуживания, [проверяют CSRF-токен](/docs/{{version}}/csrf) и выполняют другие задачи. Подробнее об этом будет рассказано далее.
