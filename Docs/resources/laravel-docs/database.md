...
Обычно подключение к базе данных настраивается с помощью нескольких значений: `host`, `database`, `username`, `password` и других. Каждому из них соответствует собственная переменная окружения. Поэтому при настройке подключения к базе данных на рабочем веб-сервере необходимо управлять несколькими переменными окружения.
...
Одним из распространенных узких мест современных веб-приложений является время выполнения запросов к базе данных. Laravel может вызвать выбранное замыкание, когда суммарное время запросов в рамках одного HTTP-запроса превысит заданный порог. Для этого передайте порог в миллисекундах и замыкание методу `whenQueryingForLongerThan`. Обычно этот метод вызывают в методе `boot` [сервис-провайдера](/docs/{{version}}/providers):
