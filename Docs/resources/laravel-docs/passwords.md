---
git: 30008730b46da57e93562c2065868a04aad8b087
---
...
Вы можете изменить URL-адрес ссылки для сброса пароля, используя метод `createUrlUsing` класса уведомлений `ResetPassword`. Этот метод принимает замыкание, которое получает экземпляр ожидающего уведомление пользователя, а также токен ссылки для сброса пароля. Как правило, этот метод следует вызывать из метода `boot` поставщика услуг `AppServiceProvider` вашего приложения:
