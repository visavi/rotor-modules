...
После установки Sail можно запустить Artisan-команду `sail:install`. Она опубликует файл `compose.yaml` в корне приложения и добавит в `.env` переменные окружения, необходимые для подключения к Docker-сервисам:
...
В файле `compose.yaml` также описан контейнер [Redis](https://redis.io). Он использует [том Docker](https://docs.docker.com/storage/volumes/), поэтому данные экземпляра Redis сохраняются даже после остановки и перезапуска контейнеров. После запуска контейнеров приложение сможет подключиться к Redis, если установить для переменной окружения `REDIS_HOST` в файле `.env` значение `redis`.
...
Внутренняя переменная окружения `XDEBUG_CONFIG` получает значение `client_host=host.docker.internal`, чтобы Xdebug был правильно настроен для macOS и Windows (WSL2). Имя хоста `host.docker.internal` доступно в Docker Desktop для macOS и Windows. Оно также поддерживается в Docker 20.10 и новее под Linux, поэтому ручная настройка не требуется.
