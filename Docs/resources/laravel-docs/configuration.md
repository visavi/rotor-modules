---
git: 6069fea96e90c34bd3c2d1beaec5eb9757d4389a
---
...
При шифровании файла окружения вы можете использовать опцию `--readable`, чтобы сохранить видимыми имена переменных, но зашифровать их значения:

```shell
php artisan env:encrypt --readable
```

В результате будет создан зашифрованный файл следующего формата:
...
Использование читаемого формата позволяет видеть, какие переменные окружения существуют, не раскрывая конфиденциальные данные. Это также значительно упрощает проверку Pull Request, поскольку можно увидеть, какие переменные были добавлены, удалены или переименованы, без необходимости расшифровывать файл.

При расшифровке файлов окружения Laravel автоматически определяет использованный формат, поэтому для команды `env:decrypt` не требуются дополнительные опции.

> [!NOTE]
> При использовании опции `--readable` комментарии и пустые строки из исходного файла окружения не включаются в зашифрованный вывод.
