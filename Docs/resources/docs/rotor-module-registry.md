...
- **Каталог** — AdminPanel → Модули → Каталог: модули из подключённых реестров скачиваются и устанавливаются автоматически. Список фильтруется по статусу, ищется по названию и сортируется по названию, статусу, версии или дате релиза (свежие сверху).
...
Каждой версии проставляется `released_at` (дата `Y-m-d`) — в тот прогон, который увидел версию впервые. Накопленная версия свою дату сохраняет, поэтому пересборка реестра не выдаёт старые релизы за свежие. Версии, вышедшие до появления поля, остаются без даты — в каталоге они уходят в конец сортировки «по дате».
