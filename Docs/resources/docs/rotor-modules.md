...
├── openapi.json              # Описание методов API (необязательно)
...
Добавляет счётчик раздела в `GET /api/stats` — ключом служит морф-имя модели. По умолчанию считаются все записи; если часть из них скрыта модерацией, передайте своё замыкание:
...
Если модуль добавляет методы API, он описывает их сам — файлом `openapi.json` в корне модуля. Ядро подмешивает описания включённых модулей в свою спецификацию и отдаёт всё вместе на странице `/api`. У выключенного модуля файла на диске нет, поэтому его раздел в документации не появляется.

Формат — обычный документ OpenAPI, но только с двумя ключами:
    ...
    "paths": {
        "/my-module": {
            "get": {
                ...
                "operationId": "getMyModule",
                "responses": {
                    "200": {
                        "description": "OK",
                        "content": {
                            "application/json": {
                                "schema": { "$ref": "#/components/schemas/MyRecord" }
                            }
                        }
                    }
                }
...
Схемы ядра (`Author`, `File`, `Comment` и другие) доступны по обычному `$ref` — они уже есть в собранном документе. Имена своих схем стоит делать уникальными: одноимённая схема другого модуля перезапишет вашу.
