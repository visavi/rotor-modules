...
Размер загружаемых файлов контролируется в `.env`:

```ini
UPLOAD_MAX_IMAGE_SIZE=2048   # KB, максимальный размер изображения
UPLOAD_MAX_FILE_SIZE=10240   # KB, максимальный размер файла
```

Также убедитесь, что `php.ini` разрешает нужный размер:
...
Движок поддерживает несколько типов капч. Настройка в AdminPanel → Настройки → Безопасность.

Для Google reCAPTCHA добавьте в `.env`:
