...
Запись отдаётся вместе с комментариями: в `data` — сама запись, в `comments` — её комментарии со своей пагинацией. Блок комментариев готовит трейт ядра `HandlesApiComments`, он же обрабатывает `per_page`, `page` и `order`:
...
Форум устроен иначе — там в `data` идут посты, а тема уходит в `additional`: у темы нет своего текста, её содержимое и есть сообщения. У записи раздела текст свой, а комментарии к нему вторичны.

Комментарии приходят лентой по дате — не деревом. Дерево и пагинация несовместимы: родитель ответа легко остаётся на другой странице. Поэтому ответ несёт краткий контекст родителя в поле `parent` (`id`, `login`, начало текста) — клиенту хватает его на строку «в ответ @login», и порядок загрузки страниц не важен. Размер страницы берётся из настройки `comments_per_page`. Удалённые остаются заглушкой с `deleted: true`.
...
Запись раздела создаётся и правится своими ручками модуля (`POST /api/offers`, `PATCH /api/offers/{id}`) — правила берите из веб-контроллера, чтобы сайт и API не расходились: те же настройки длин, те же проверки прав, тот же антифлуд. Вложения, загруженные до создания записи, привязывает `FileService::attachPending($model)` из ядра.
...
### Настройки модуля в /api/config

Клиенту нужны лимиты форм — длины заголовка и текста. Ядро их не знает: настройки лежат в общей таблице плоским списком, и по имени не отличить лимит формы от пароля почты. Поэтому модуль объявляет, что отдавать, в секции `api` файла `module.php`:

```php
'api' => [
    'forum' => [
        'title_min' => 'forum_title_min',   // строка — имя настройки
        'title_max' => 'forum_title_max',
        'answers_min' => 2,                 // не строка — значение как есть
    ],
],
```

Секция появится в `/api/config` под своим ключом, когда модуль включён, и исчезнет вместе с ним. Значения не кешируются вместе с настройками ядра — список меняется сразу после включения модуля.
