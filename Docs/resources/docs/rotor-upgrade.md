...
## Порядок между мажорами

Мажор ставится только с последней версии предыдущего мажора: сначала 14.x до последнего релиза, потом 15.0. Промежуточные миноры несут мосты совместимости и upgrade-миграции, поэтому прыжок 14.0 → 15.0 оставит сайт с модулями, собранными под несуществующий уже API, — упадёт не страница, а весь сайт, потому что модули поднимаются на старте.

Панель обновлений это соблюдает сама: пока сайт не дошёл до конца своей линии, релизы следующего мажора в списке не показываются, а вместо них выводится, до какой версии нужно обновиться сначала.

Когда мажорный релиз становится доступен, на его карточке перечисляются установленные модули, собранные под текущий мажор (`requires` ниже целевой версии). Обновление это не блокирует — `requires` задаёт только нижнюю границу, — но такие модули нужно обновить сразу после перехода.
