    ...
    <div class="section-form mb-3 shadow">
        <form action="/admin/delivery" method="post">
            ...
            <div class="form-check">
                ...
                <label class="form-check-label" for="inputType1">{{ __('delivery::delivery.online') }}</label>
            </div>
            <div class="form-check">
                <input class="form-check-input" type="radio" id="inputType2" name="type" value="2"{{ $inputType === 2 ? ' checked' : '' }}>
                <label class="form-check-label" for="inputType2">{{ __('delivery::delivery.active') }}</label>
            </div>
            <div class="form-check">
                <input class="form-check-input" type="radio" id="inputType3" name="type" value="3"{{ $inputType === 3 ? ' checked' : '' }}>
                <label class="form-check-label" for="inputType3">{{ __('delivery::delivery.admins') }}</label>
            </div>
            <div class="form-check">
                <input class="form-check-input" type="radio" id="inputType4" name="type" value="4"{{ $inputType === 4 ? ' checked' : '' }}>
                <label class="form-check-label" for="inputType4">{{ __('delivery::delivery.users') }}</label>
            </div>
