<div class="mb-3{{ hasError('answers') }}">
    <div class="js-answer-list">
        ...
        <label for="answers0" class="form-label">{{ __('forum::forums.vote_options') }}:</label>
        ...
        @foreach ($answers as $key => $answer)
            @if ($loop->index < max(2, count($vote->getAnswers ?? [])))
                <div class="input-group mt-1">
                    <input type="text" name="answers[{{ $key }}]" class="form-control" id="answers{{ $loop->index }}" value="{{ $answer }}" maxlength="50" placeholder="{{ __('forum::forums.answer') }}">
                </div>
            @else
                <div class="input-group mt-1 js-answer-append">
                    <input class="form-control" name="answers[{{ $key }}]" type="text" value="{{ $answer }}" maxlength="50" placeholder="{{ __('forum::forums.answer') }}">
    ...
    <script type="module">
        ...
        document.querySelector('.js-answer-add')?.addEventListener('click', function (e) {
            ...
            if (answerList.querySelectorAll('input').length >= 10) {
                notyf.error("{{ __('forum::forums.vote_hint_answers') }}");
                return;
            }
            answerList.insertAdjacentHTML('beforeend',
                '<div class="input-group mt-1 js-answer-append">' +
                '<input class="form-control" name="answers[]" type="text" value="" maxlength="50" placeholder="<?= __('forum::forums.answer') ?>">' +
                '<span class="input-group-text">' +
                '<a class="js-answer-remove" href="#"><i class="fa fa-times"></i></a>' +
                '</span>' +
                '</div>');
