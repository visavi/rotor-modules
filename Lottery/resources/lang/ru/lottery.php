<?php

return [
    'title'                  => 'Лотерея',
    'lottery_info'           => 'Участвуй в лотерее! С каждым разом джек-пот растет<br>
            Стань счастливым обладателем заветной суммы!',
    'jackpot_amount'         => 'Джек-пот составляет: :jackpot',
    'winning_number'         => 'Выигрышное число прошлого тура: :number',
    'winners'                => 'Победители',
    'jackpot_not_win'        => 'Джек-пот не выиграл никто!',
    'enter_number'           => 'Введите число',
    'enter_number_inclusive' => 'Введите число от :min до :max включительно',
    'buy_ticket'             => 'Купить билет',
    'participate'            => 'В этом туре участвуют',
    'ticket_price'           => 'Стоимость билета',
    'in_stock'               => 'В наличии',
    'participants'           => 'Участники',
    'rate'                   => 'Ставка',
    'no_participants'        => 'Участников еще нет!',

    'lottery_not_activated'    => 'Лотерея еще не активирована!',
    'already_bought_ticket'    => 'Вы уже купили билет! Нельзя покупать дважды!',
    'no_money'                 => 'У вас недостаточно средств!',
    'must_enter_number'        => 'Необходимо ввести число!',
    'ticket_success_purchased' => 'Билет успешно приобретен!',
    'congratulations_winning'  => 'Поздравляем! Вы сорвали [url=/lottery]Джек-пот в лотерее[/url] и выиграли :jackpot',
    'ticket_purchased'         => 'Билет куплен, ваша ставка: :number',
];
