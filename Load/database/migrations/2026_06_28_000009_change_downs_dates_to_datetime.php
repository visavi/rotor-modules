...
return new class extends Migration {
    ...
    {
        if (Schema::getColumnType('downs', 'created_at') === 'datetime') {
            return;
        }
        ...
        $this->convert('downs', ['created_at', 'updated_at'], static fn ($r) => [
            'created_at_dt' => $toDt($r->created_at),
            'updated_at_dt' => $toDt($r->updated_at),
        ]);
        $indexes = $this->indexNames('downs', [['created_at']]);
        Schema::table('downs', function (Blueprint $table) use ($indexes) {
        ...
        Schema::table('downs', fn (Blueprint $table) => $table->index('created_at'));
    }
