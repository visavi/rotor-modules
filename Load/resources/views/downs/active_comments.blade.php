...
@section('title', sprintf('%s - %s (%s)', __('load::loads.loads'), __('load::loads.active_comments', ['user' => $user->getName()]), __('main.page_num', ['page' => $comments->currentPage()])))

@section('header')
    <h1>{{ __('load::loads.active_comments', ['user' => $user->getName()]) }}</h1>
@stop
...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('load::loads.active_comments', ['user' => $user->getName()]) }}</li>
        </ol>
    </nav>
@stop
