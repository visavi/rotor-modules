...
@section('title', __('load::loads.view_file') . ' ' . $document['name'])
...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('downs.zip', ['id' => $down->id, 'fid' => $file->id]) }}">{{ __('load::loads.view_archive') }}</a></li>
            <li class="breadcrumb-item active">{{ __('load::loads.view_file') }}</li>
        </ol>
    </nav>
@stop

@section('content')
    {{ __('load::loads.file_size') }}: {{ formatSize($document['size']) }}
    <hr>
    ...
    @else
        {{ showError(__('load::loads.empty_file')) }}
    @endif
@stop
