...
@section('title', sprintf('%s - %s (%s)', __('load::loads.loads'), __('load::loads.new_comments'), __('main.page_num', ['page' => $comments->currentPage()])))

@section('header')
    <h1>{{ __('load::loads.new_comments') }}</h1>
@stop
...
@section('breadcrumb')
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{ __('load::loads.new_comments') }}</li>
        </ol>
    </nav>
@stop
