...
@section('title', sprintf('%s (%s)', __('load::loads.new_publications'), __('main.page_num', ['page' => $downs->currentPage()])))

@section('header')
    <h1>{{ __('load::loads.new_publications') }}</h1>
@stop
            ...
            <div class="section mb-3 shadow">
                <div class="section-header d-flex align-items-start">
                    ...
                    <div class="text-end">
                        ...
                        @if (isAdmin('boss'))
                            <form action="{{ route('admin.downs.delete', ['id' => $data->id]) }}" method="post" class="d-inline" onsubmit="return confirm('{{ __('load::loads.confirm_delete_down') }}')">
                                @csrf
                                @method('DELETE')
                ...
                <div class="section-content">
                    {{ __('load::loads.load') }}: <a href="{{ route('admin.loads.load', ['id' => $data->category->id]) }}">{{ $data->category->name }}</a><br>
                    {{ __('load::loads.files_images') }}: {{ $data->getFiles()->count() }}/{{ $data->getMedia()->count() }}<br>
