...
Модули необходимо устанавливать внутри директории /modules
Путь к модулю будет такой /modules/ИмяМодуля

Для включения модуля необходимо в админ-панели перейти в раздел Модули, выбрать нужный модуль и включить его, при включении модуля будут автоматически выполнены все миграции и созданы ссылки на статические файлы или директории, будут подключены настройки, хуки и остальные компоненты

При отключении модуля он становится недоступен, миграции в БД остаются без изменений
После включения модуль будет работать так же как и до выключения, все настройки, записи и другие данные останутся нетронутыми

При удалении модуля, все миграции будут возвращены в первоначальное состояние, то есть созданные таблицы будут удалены, а также будут удалены ссылки на статичные файлы и все остальные данные
...
- panel - Ссылка или массив ссылок на страницы модуля для администратора

#### Controllers
Контроллеры с пространством имен namespace Modules\ИмяМодуля\Controllers;

Контроллеры должны быть наследованы от \App\Http\Controllers\Controller или \App\Http\Controllers\Admin\AdminController

#### Models
Модели с пространством имен namespace Modules\ИмяМодуля\Models;

Модели должны быть наследованы от \Illuminate\Database\Eloquent\Model
...
### Статические файлы
resources/assets - директория для статических файлов или директорий со статическими файлами, это могут быть картинки, css, js и другие файлы которые должны быть доступны напрямую, при установке модуля автоматически создается ссылка, доступная по адресу /assets/modules/преобразованноеИмяМодуля 
